<template>
    <div>
        ...
        <div class='morecity'>
           <h3>更多城市</h3>
            <ul>
                <li v-for='c,i in intercityList' :key='i' @click='jump(i)' >           
                        {{c.firstChar}}                  
                </li>
            </ul>
        </div>
        <div class='listcity' v-for='t,i in intercityList'  :key='i' >
            <h3>{{t.firstChar}}</h3>
            <ul>
...
export default {
   components:{cityhead},
    ...
    computed:{
        ...mapState(['intercityList',])
    },
    methods:{
        ...mapActions(['getDataIntercity']),
